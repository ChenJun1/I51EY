package com.cvnavi.logistics.i51eyun.app.extra.datepicker;
...
public class WheelView extends View {
     ...
     * 顶部和底部的阴影颜色
     */
    private static final int[] SHADOWS_COLORS = new int[]{0xffffffff, 0x96ffffff, 0x00ffffff};
    ...
    /**
     * 普通条目布局 StaticLayout 布局用于控制 TextView 组件, 一般情况下不会直接使用该组件, 除非你自定义一个组件 或者
     * 想要直接调用 Canvas.drawText() 方法
     */
    ...
    /**
     * Scroll 类封装了滚动动作. 开发者可以使用 Scroll 或者 Scroll 实现类 去收集产生一个滚动动画所需要的数据,
     * 返回一个急冲滑动的手势. 该对象可以追踪随着时间推移滚动的偏移量, 但是这些对象不会自动向 View 对象提供这些位置.
    ...
    private void initResourcesIfNecessary() {
        ...
        if (centerDrawable == null) {
            centerDrawable = getContext().getResources().getDrawable(R.drawable.shape_wheelview_stroke_ededed);
        }
		 ...
		 * 设置 View 组件的背景
		 */
        setBackgroundResource(R.drawable.shape_wheelview_solid_fdfdfd_stroke_ededed);
    }
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (maxLength > 0) {
			/*
			 * 使用方法 FloatMath.ceil() 方法有以下警告 Use java.lang.Math#ceil instead of
			 * android.util.FloatMath#ceil() since it is faster as of API 8
			 */
		...
		/*
		 * 重新计算宽度 , 如果宽度是给定的宽度, 不是我们计算出来的宽度, 需要重新进行计算 重新计算的宽度是用于
		 * 
    ...
    private void drawValue(Canvas canvas) {
        valuePaint.setColor(Color.rgb(58, 149, 230)); // Color.rgb(255, 0, 48)
    ...
    private void drawItems(Canvas canvas) {
        ...
        // 设置画笔颜色
        itemsPaint.setColor(Color.rgb(136, 136, 136));
        // 将当前 View 状态属性值 转为整型集合, 赋值给 普通条目布局的绘制属性
    ...
    private void doScroll(int delta) {
		...
		/*
		 * pos 是滚动后的目标元素索引 计算当前位置, 当前条目数 减去 滚动的条目数 注意 滚动条目数可正 可负
		 */
		...
		/*
		 * 如果当前位置不是滚动后的目标位置, 就将当前位置设置为目标位置 否则就重绘组件
		 */
        if (pos != currentItem) {
    ...
    private void justify() {
		...
		/*
		 * 当滚动补偿 大于 0, 说明还有没有滚动的部分, needToIncrease 是 当前条目是否小于条目数 如果 滚动补偿不大于 0,
		 * needToIncrease 是当前条目是否大于 0
		 */
