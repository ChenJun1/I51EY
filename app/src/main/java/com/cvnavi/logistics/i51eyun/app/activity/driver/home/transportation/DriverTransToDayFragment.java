 ...
 */
public class DriverTransToDayFragment extends BaseFragment implements OnClickItemListener {
    ...
    public static DriverTransToDayFragment getInstance() {
        return new DriverTransToDayFragment();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new DriverTransListViewAdapter(dataList, getActivity(), DriverTransToDayFragment.this);
        ...
        pullRefreshList.setOnRefreshListener(new PullToRefreshBase.OnRefreshListener2<ListView>() {
            ...
            public void onPullDownToRefresh(PullToRefreshBase<ListView> refreshView) {
                ...
                //执行刷新
                getSchedulingList(DateUtil.getLastNDays(0), DateUtil.getLastNDays(0), false);
            }
...
//                    datavalue.CarCode_Date = carInfo.CarCode_Date;
//                    datavalue.CarCodeSerial_Oid = carInfo.CarCodeSerial_Oid;
//                    datavalue.Line_Oid = carInfo.Line_Oid;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case Constants.REQUEST_SUCC:
                    isDelect = true;
                    getSchedulingList(DateUtil.getLastNDays(0), DateUtil.getLastNDays(0), true);
    ...
    private void getSchedulingList(String beginDate, String endDate, boolean showDia) {
        ...
        LogUtil.d("-->>Today request=" + new Gson().toJson(dataRequestBase));
        VolleyManager.newInstance().PostJsonRequest(TMSService.GetShiftList_TAG, TMSService.GetShiftList_Request_Url, GsonUtil.newInstance().toJson(dataRequestBase), new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                LogUtil.d("-->>Today respon=" + response.toString());
...
//                    pullRefreshList.setAdapter(adapter);
    ...
    public void onResume() {
        super.onResume();
        LogUtil.d("-->> onResume");
        DriverTransportationActivity activity = (DriverTransportationActivity) getActivity();
        LogUtil.d("-->> activity.beginDate=" + activity.beginDate);
        isRefresh = true;
        if (activity.beginDate != null) {
            getSchedulingList(activity.beginDate, activity.endDate, true);
        } else {
